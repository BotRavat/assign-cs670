                            ...
                            int n, int k, int modValue)
{
    ...
    // Use shared_ptr sockets so coroutines can safely extend their lifetime.
    auto socket_p0 = std::make_shared<tcp::socket>(io);
    auto socket_p1 = std::make_shared<tcp::socket>(io);
    ...
    for (size_t qi = 0; qi < queries.size(); qi++)
    {
        ...
        // -------- Generate data for this query (local copies) --------
        ...
        vector<int> e(n, 0);
        if (query_j >= 0 && query_j < n) e[query_j] = 1;
        ...
        // -------- Make stable local copies (one-per-query) for P0 --------
        ...
        // -------- Make stable local copies (one-per-query) for P1 --------
        ...
        // copies of DPF data for captures
        auto rootSeed_copy  = rootSeed;
        auto T0_copy        = T0;
        auto T1_copy        = T1;
        auto CW_copy        = CW;
        auto FCW0_copy      = FCW0;
        ...
        // -------- Spawn send coroutine for P0 (capture socket shared_ptr by value) --------
        co_spawn(io,
                 [p0 = socket_p0,
                  n, k, modValue,
                  ...
                  one0_copy, query_i,
                  rootSeed_copy, T0_copy, CW_copy, FCW0_copy]() mutable -> awaitable<void>
                 {
                     try {
                         ...
                         cerr << "Dealer: exception in send to P0 lambda: " << e.what() << "\n";
                     }
                     co_return;
                 },
                 detached);

        // -------- Spawn send coroutine for P1 --------
        co_spawn(io,
                 [p1 = socket_p1,
                  n, k, modValue,
                  ...
                  one1_copy, query_i,
                  rootSeed_copy, T1_copy, CW_copy, FCW1_copy]() mutable -> awaitable<void>
                 {
                     try {
                         co_await send_shares_to_party(
                             *p1,
                         ...
                         cerr << "Dealer: exception in send to P1 lambda: " << e.what() << "\n";
                     }
                     co_return;
                 },
                 detached);

        // -------- Wait and receive final shares (use dereferenced shared_ptr) --------
    ...
    // Close sockets (shared_ptr will release)
    boost::system::error_code ec;
    (*socket_p0).shutdown(tcp::socket::shutdown_both, ec);
    (*socket_p0).close(ec);
    (*socket_p1).shutdown(tcp::socket::shutdown_both, ec);
    (*socket_p1).close(ec);
