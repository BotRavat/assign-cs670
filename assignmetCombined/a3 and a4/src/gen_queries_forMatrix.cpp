...
static inline uint64_t mod64(u128 x)
{
    return (uint64_t)(x & 0xFFFFFFFFFFFFFFFFULL); // ★ FIX #1 (correct modulo 2^64)
}
...
void EvalFull(DpfKey &dpf, int dpf_size, int totaldpfs)
{
    ...
    for (int i = 0; i < dpf_size; i++)
    {
        for (int j = 0; j < rowLength; j++)
        {
            eval1[i][j] = (u128)(-(int64_t)mod64(eval1[i][j])); // ★ FIX #2 (signed modular negation)
        }
    }
    ...
    for (int i = 0; i < dpf_size; i++)
    {
        for (int j = 0; j < rowLength; j++)
        {
            ...
            cout << (uint64_t)mod64(eval0[i][j] + eval1[i][j]) << " ";
            cout << " ";
        }
        cout << endl;
    }
...
void generateDPF(DpfKey &dpf, int domainSize)
{
    ...
    for (int i = 0; i < k; i++)
    {
        dpf.FCW0[i] = mod64(leafVecAtTarget0[i] - leafVecAtTarget1[i] + dpf.FCW1[i]);
    }
    for (int i = 0; i < k; i++)
    {
        dpf.FCW1[i] = (u128)(-(int64_t)mod64(dpf.FCW1[i]));
    }
}
